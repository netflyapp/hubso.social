  ...
  return (
    <Section title={t.pricing.title} subtitle={t.pricing.subtitle}>
      <div className="flex justify-center mb-10">
        <span className="mr-2 font-semibold">{t.pricing.monthly}</span>
        <label className="relative inline-flex items-center cursor-pointer">
          <Label>
            <Switch checked={!isMonthly} onCheckedChange={handleToggle} />
          </Label>
        </label>
        <span className="ml-2 font-semibold">{t.pricing.yearly}</span>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {siteConfig.pricing.map((plan, index) => {
          const planT = t.pricing.plans[index];
          return (
          <motion.div
            key={index}
          ...
          >
            ...
            <div>
              ...
              <ul className="mt-5 gap-2 flex flex-col">
                {(planT?.features ?? plan.features).map((feature, idx) => (
              ...
              >
                {planT?.buttonText ?? plan.buttonText}
              </Link>
              <p className="mt-6 text-xs leading-5 text-muted-foreground">
                {planT?.description ?? plan.description}
              </p>
            </div>
          </motion.div>
          );
