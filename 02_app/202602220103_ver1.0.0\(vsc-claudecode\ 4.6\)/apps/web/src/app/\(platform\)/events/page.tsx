        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 space-y-6">
            ...
            {mockEvents.map((event, idx) => (
              ...
              >
                <div className="flex">
                  <div className={`w-20 sm:w-24 bg-gradient-to-b ${eventGradients[idx % 6]} flex flex-col items-center justify-center text-white shrink-0 py-4`}>
                    <span className="text-xs font-semibold uppercase">{event.month}</span>
                    <span className="text-3xl font-extrabold leading-none mt-1">{event.day}</span>
                    <span className="text-[10px] mt-1 opacity-80">{event.time}</span>
                  </div>
                  <div className="p-4 flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`px-2 py-0.5 ${eventStatusColors[idx % 6].bg} ${eventStatusColors[idx % 6].darkBg} ${eventStatusColors[idx % 6].text} ${eventStatusColors[idx % 6].dark} text-[10px] font-bold rounded-full flex items-center gap-1`}>
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      ...
                      <a 
                        href="#" 
                        className={`ml-auto h-7 px-3 ${eventButtonColors[idx % 6]} text-white text-xs font-semibold rounded-lg transition-colors flex items-center gap-1`}
                      >
                        <Icon icon="solar:play-bold" width={12} height={12} />
                        {event.action || 'Dołącz'}
                      </a>
                    </div>
                  </div>
                ...
                {mockEvents.map((event, idx) => (
                  <div key={idx} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-slate-400">{event.month} {event.day} · {event.time}</div>
                    </div>
                  </div>
                ))}
              </div>
